...
export class BcgMapOverlay extends ScopedElementsMixin(BcgModule) {
  ...
  @property({type: Function}) actionButtonCallback = () => { this.showOverlay = true };
  @property({type: Function}) closeButtonCallback = () => { this.showOverlay = false };
  ...
  render() {
    ...
    <div class="map-wrapper">
      ...
      <bcg-button class="action-button" variant="secondary" @click=${() => this.actionButtonCallback()}>${this.actionButtonLabel}</bcg-button>

      ${this.showOverlay ? html`
      <div class="overlay-wrapper" style="width: ${this.overlayWidth}">
        <bcg-overlay .closeButtonCallback=${() => this.closeButtonCallback()}>
